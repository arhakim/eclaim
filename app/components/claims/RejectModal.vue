...
const { rejectClaim } = useApprovals()
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  loading.value = true
  try {
    const success = await rejectClaim(props.claim.id, authStore.user.id, event.data.comments)
...
<template>
  <UModal
    ...
    description="Provide a reason for rejecting this expense claim"
    :ui="{
      content: 'max-w-4xl'
    }"
  >
      ...
      >
        <div v-if="claim" class="space-y-4">
          ...
          <UAlert
            ...
            variant="subtle"
            title="Reject Expense Claim"
            description="This action will reject the claim and notify the employee. Please provide a clear reason for rejection."
          />
          ...
          >
            <UFormField
              ...
              name="comments"
              required
              help="This reason will be shared with the employee to help them understand why the claim was rejected."
            >
              <UTextarea
